...
def readin_virus_config(virus):
    ...
    with open(config_json) as json_handle:
        ...
        configs["alignment_file"] = configs["alignment_file"].replace("host", host)
        configs["reference_file"] = configs["reference_file"].replace("host", host)
...
def save_organized_data(host, virus, subtype, gene, window, min_seqs, year_max, year_min):
    ...
    save_filename = f'{filepath}input_data_{gene}_{window}_{min_seqs}.json'
...
def get_alignment_outgroup_data(host, virus, subtype, gene, window, min_seqs, year_max, year_min):
    ...
    saved_data_name = f'{filepath}input_data_{gene}_{window}_{min_seqs}.json'
...
def run_adaptation_analysis(host, virus, subtype, genes, window, min_seqs, year_max, year_min,
    ...
    for gene in genes:
        ...
        if bootstrap:
            save_json_name = f'{gene}_{window}_{min_seqs}_adaptation_bootstrapped.json'
            ...
            save_json_file = f'{gene}_{window}_{min_seqs}_adaptation.json'
            save_json_folder = f"adaptation_results/results/{host}"
            save_json_full_path = f"{save_json_folder}/{save_json_name}"
...
#     
#     configs = readin_virus_config(virus)
#     genes_to_analyze = configs["genes"]
# 
#     
...
#         print(f"  Alignment exists: {os.path.exists(alignment_file)}")
#         print(f"  Meta exists: {os.path.exists(meta_file)}")
#         
#         if not os.path.exists(alignment_file) or not os.path.exists(meta_file):
#             print(f" Skipping {gene} - file not found")
#             continue
#         
#         # Check sequence count
#         seq_count = len(list(SeqIO.parse(alignment_file, "fasta")))
#         print(f"  Sequences in alignment: {seq_count}")
#         
#         if seq_count < min_seqs:
#             print(f" Not enough sequences (need {min_seqs})")
#             continue
#         
#         # Try to get year windows
#         try:
#             (aligned_codons_in_window, outgroup_codons, year_windows) = get_alignment_outgroup_data(host, virus, subtype, gene, 
#                                                                                                      window, min_seqs, year_max, year_min)
#             print(f"  ✓ {len(year_windows)} time windows created")
...
hosts = ["canine_h3n2", "swine_euro", "swine_na", "human", "equine"]

# avian had to be done separately because their NAs are split into N2 and N8
